...
## 待决策清单处理结果（2026-09-06 用户决策 + 落地实施）

| #   | 决策项                     | 结论                                               | 落地                                                                                                                                                 |
| --- | -------------------------- | -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | H5 scan_token 存储按码隔离 | 现在做                                             | `864f0bf9` 后：按码键 `yimatong:scan-token:{publicId}` + 激活码作用域 + 旧全局键清理                                                                 |
| 2   | OAuth 成功回跳自动续领     | 现在做                                             | sessionStorage 续领意图（TTL 10 分钟、用后即焚、权益匹配），回跳自动领取                                                                             |
| 3   | 领取链路 IP 绑定           | **保持现状**（安全优先：凭证转发无法在他网络冒领） | 无代码改动                                                                                                                                           |
| 4   | 导出排除已作废码           | 现在做                                             | `exclude_voided` 请求字段 + `exported_item_count` 迁移 + 补印重导出 + 审计/响应头标注 + Admin 弹窗流                                                 |
| 5   | analytics 日切时区         | **改 Asia/Shanghai**                               | `app/utils/stats_clock.py` 唯一口径源；全部分析域切换；`python -m app.cli stats recompute` 重算 CLI（演示库 269 租户日实测，事件数与聚合数对账一致） |
| 6   | 非微信 UA 降级页           | 简单引导页                                         | 非微信 UA 单码/内码 HTML 路径返回「请使用微信扫一扫」固定引导页                                                                                      |
| 7   | CRM 同步立项               | **暂不立项**                                       | 保持诚实空态                                                                                                                                         |
| 8   | 多工作区演示账号语义       | 修预填 slug                                        | 「代运营工作台」按钮直登 demo-agency（agency_admin@demo.com），各账号显式 tenantSlug                                                                 |
| 9   | access_token XSS 窗口      | **维持现状**（依赖 CSP）                           | 无代码改动                                                                                                                                           |
| 10  | 平台审计日志分页           | 现在做                                             | 端点改 `items/total/page/page_size` 信封；平台前端服务端分页                                                                                         |

4 个建议专项全部立项实施：

| 专项                               | 落地                                                                                             |
| ---------------------------------- | ------------------------------------------------------------------------------------------------ |
| channels 页拆分 + 死代码清理       | 3007 行 page.tsx 拆为 hook + shared + 6 Tab + Modals（页面 130 行编排）；删除 633 行无引用旧组件 |
| admin vitest 并行 flaky 根治       | CI 环境 `maxWorkers=2` + `ci.yml` 按 `--shard` 分两片；实测 408 用例 0 失败（此前同环境 6 失败） |
| i18n/private-domain 权限与审计补全 | 写路由统一 `tenant:manage` 门禁 + 平台审计；审计动作字典补全；i18n 页非管理员只读                |
| 风控规则编辑/删除 UI + 配置模板    | RulesTab 支持编辑（PATCH+乐观锁）、软删（诚实文案）、类型模板自动填充                            |

### 收尾回归（最终状态）

- 后端全量 `pytest`、前端 `pnpm check`、三应用 `tsc`/`vitest`/`build` 详见各模块提交；admin vitest 分片 408 用例 0 失败
- analytics 切换后对演示库执行 `stats recompute --apply`：`daily_scan_stats` 聚合总数与 `scan_events` 事件数完全对账（diff=0）
