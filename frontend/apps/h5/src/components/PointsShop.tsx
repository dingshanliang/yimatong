...
export function PointsShop({
  ...
  if (!consumerId) {
    return (
      <div className="rounded-2xl border border-warning bg-warning-bg p-4 text-center">
        <p className="text-sm font-medium text-warning">
          完成手机号留资后可兑换积分权益
        </p>
        <p className="mt-1 text-xs text-warning">
          留资成功后，积分余额和可兑换商品会自动显示。
        </p>
      </div>
    );
  }
  ...
  if (products.length === 0) {
    return (
      <div className="py-8 text-center">
        <p className="text-sm text-foreground-tertiary">暂无可兑换商品</p>
      </div>
    );
  }
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0">
                ...
                <p className="mt-1 text-xs text-foreground-tertiary">
                  有效期 {formatDate(product.starts_at)} 至{" "}
                  {formatDate(product.ends_at)}
                </p>
                <p className="mt-1 text-xs text-foreground-tertiary">
                  每人限兑{" "}
                  {product.per_consumer_limit && product.per_consumer_limit > 0
                    ? `${product.per_consumer_limit} 次`
                    : "不限制"}
                </p>
              </div>
            ...
            {isExchanged ? (
              <div className="mt-3 rounded-xl bg-success-bg py-2 text-center text-sm text-success">
                ✅ 已兑换
              </div>
            ) : (
              <button
              ...
              >
                {product.stock <= 0
                  ? "已售罄"
                  : !canExchange
                    ? product.exchange_block_reason || "暂不可兑换"
                    : isExchanging
                      ? "兑换中..."
                      : "立即兑换"}
              </button>
