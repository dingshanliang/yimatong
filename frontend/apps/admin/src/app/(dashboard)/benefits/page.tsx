...
function summarizeVisibleBenefits(
  benefits: Benefit[],
  total: number,
  claimCount: number
): BenefitSummary {
  const stockTotal = benefits.reduce(
    (sum, benefit) => sum + (benefit.stock_total || 0),
    0
  );
  const stockUsed = benefits.reduce(
    (sum, benefit) => sum + (benefit.stock_used || 0),
    0
  );
  return {
    ...DEFAULT_SUMMARY,
...
export default function BenefitsPage() {
  ...
  const [contextCampaignId, setContextCampaignId] = useState<string | null>(
    null
  );
  ...
  const [deliveryDetail, setDeliveryDetail] = useState<BenefitDelivery | null>(
    null
  );
  ...
  const selectedCampaign = contextCampaignId
    ? campaigns.find((campaign) => campaign.id === contextCampaignId)
    : undefined;
  const campaignMap = useMemo(
    () =>
      campaigns.reduce<Record<string, string>>((acc, c) => {
        acc[c.id] = c.name;
        return acc;
      }, {}),
    [campaigns]
  ...
  const benefitMap = useMemo(
    () =>
      benefits.reduce<Record<string, string>>((acc, b) => {
        acc[b.id] = b.name;
        return acc;
      }, {}),
    [benefits]
  ...
  const displaySummary = useMemo(
    () =>
      summaryUnavailable
        ? summarizeVisibleBenefits(benefits, benefitsTotal, claimsTotal)
        : summary,
  ...
  const openCreate = () => {
    ...
    form.setFieldsValue({
      ...
      per_person_limit: 1,
      config_json: {
        validity_type: contextCampaignId ? "campaign_period" : undefined,
      },
    });
    setModalOpen(true);
  };
    ...
    if (record.benefit_type === "cash_red_packet") {
      for (const field of [
        ...
        if (typeof configJson[field] === "number")
          configJson[field] = fenToYuan(configJson[field] as number);
      }
    }
    form.setFieldsValue({
      name: record.name,
  ...
  const handleSubmit = async (values: Record<string, unknown>) => {
    ...
    try {
      const configJson = {
        ...((values.config_json as Record<string, number | string | boolean>) ||
          {}),
      };
      if (values.benefit_type === "cash_red_packet") {
        for (const field of [
          ...
          if (typeof configJson[field] === "number")
            configJson[field] = yuanToFen(configJson[field] as number);
        }
      }
      const payload = {
        name: values.name,
  ...
  const handleUseInCampaign = async (record: Benefit) => {
    ...
    try {
      await api.post(
        `/campaigns/${contextCampaignId}/benefits/${record.id}/attach`
      );
  ...
  const handleValuesChange = (changed: Record<string, unknown>) => {
    ...
    if (changed.benefit_type) {
      ...
      if (!nameManuallyEdited && !editItem) {
        form.setFieldsValue({
          name: benefitNameSuggestion(nextType, selectedCampaign?.name),
        });
      }
    }
  };
  ...
  const getBenefitValidityText = (record: Benefit) => {
    ...
    if (record.config_json?.validity_type === "campaign_period")
      return "随活动期有效";
    if (typeof record.config_json?.validity_days === "number")
      return `领取后 ${record.config_json.validity_days} 天内有效`;
    return "未设置";
  };
  ...
  const benefitColumns: ColumnsType<Benefit> = [
    ...
    },
    {
      ...
      render: (v: string | null) => {
        ...
        const campaignName =
          contextCampaignId && v === contextCampaignId
            ? "当前活动"
            : campaignMap[v] || "未找到活动";
        return (
          <Text ellipsis={{ tooltip: campaignName }} className="block max-w-44">
            {campaignName}
          </Text>
        );
      },
        ...
        return (
          <div className="min-w-36">
            <div className="mb-1 text-xs text-text-muted">
              已领 {record.stock_used} / {record.stock_total}，剩余 {remaining}
            </div>
            <Progress
              percent={percent}
              size="small"
    ...
    {
      title: "每位限领",
      dataIndex: "per_person_limit",
      key: "per_person_limit",
      width: 96,
    },
    {
      title: "状态",
        ...
        <Space wrap>
          {contextCampaignId && !record.campaign_id && (
            <Button
              size="small"
              type="primary"
              onClick={() => handleUseInCampaign(record)}
            >
              用于此活动
            </Button>
          )}
          {contextCampaignId && record.campaign_id === contextCampaignId && (
            <Popconfirm
              title="确认取消当前活动使用此权益？"
              onConfirm={() => handleDetachFromCampaign(record)}
            >
  ...
  const claimColumns: ColumnsType<BenefitClaim> = [
    {
      ...
      key: "consumer_id",
      render: (v: string) => renderCopyableText(v, 16),
    },
    {
      title: "权益",
      ...
      render: (_: unknown, record) =>
        renderCopyableText(
          record.campaign_name ||
            (record.campaign_id
              ? campaignMap[record.campaign_id] || record.campaign_id
              : "独立权益"),
          20
        ),
    },
    ...
    },
    {
      ...
      render: (_: unknown, record) => (
        <Button
          size="small"
          icon={<EyeOutlined />}
          onClick={() => openClaimDetail(record)}
        >
          查看详情
        </Button>
      ),
  ...
  return (
    <div className="space-y-4">
      ...
      <Row gutter={12}>
        <Col xs={12} md={6}>
          <Card size="small">
            <Statistic title="权益总数" value={displaySummary.total} />
          </Card>
        </Col>
        <Col xs={12} md={6}>
          <Card size="small">
            <Statistic title="启用中" value={displaySummary.active} />
          </Card>
        </Col>
        <Col xs={12} md={6}>
          <Card size="small">
            <Statistic
              title="剩余库存"
              value={displaySummary.stock_remaining}
              suffix={`/ ${displaySummary.stock_total}`}
            />
          </Card>
        </Col>
        <Col xs={12} md={6}>
          <Card size="small">
            <Statistic title="领取记录" value={displaySummary.claim_count} />
          </Card>
        </Col>
      </Row>
              ...
              <Card size="small">
                <div className="mb-3 flex flex-wrap items-center gap-2">
                  ...
                  <Select
                    ...
                    onChange={(benefit_type) =>
                      applyFilters({ ...filters, benefit_type })
                    }
                    options={Object.entries(BENEFIT_TYPE_MAP).map(
                  ...
                  <Select
                    ...
                    options={Object.entries(BENEFIT_STATUS_MAP).map(
                      ([value, info]) => ({ value, label: info.label })
                    )}
                  />
                  <Select
              ...
              <Card size="small">
                <div className="mb-3 flex flex-wrap items-center gap-2">
                  ...
                  <Select
                    ...
                    onChange={(benefit_id) =>
                      applyClaimFilters({ ...claimFilters, benefit_id })
                    }
                  ...
                  <Select
                    ...
                    onChange={(campaign_id) =>
                      applyClaimFilters({ ...claimFilters, campaign_id })
                    }
                  ...
                  <Select
                    ...
                    onChange={(status) =>
                      applyClaimFilters({ ...claimFilters, status })
                    }
                    options={[
                  ...
                  <Select
                    ...
                    onChange={(delivery_status) =>
                      applyClaimFilters({ ...claimFilters, delivery_status })
                    }
                    options={Object.entries(DELIVERY_STATUS_MAP).map(
                      ([value, info]) => ({ value, label: info.label })
                    )}
                  />
          ...
          <Space orientation="vertical" size="middle" className="w-full">
            <Descriptions size="small" column={1} bordered>
              <Descriptions.Item label="消费者">
                {renderCopyableText(selectedClaim.consumer_id, 28)}
              </Descriptions.Item>
              <Descriptions.Item label="权益">
                {selectedClaim.benefit_name ||
                  benefitMap[selectedClaim.benefit_id] ||
                  selectedClaim.benefit_id}
              </Descriptions.Item>
              <Descriptions.Item label="活动">
                {selectedClaim.campaign_name ||
                  (selectedClaim.campaign_id
                    ? campaignMap[selectedClaim.campaign_id] ||
                      selectedClaim.campaign_id
                    : "独立权益")}
              </Descriptions.Item>
              ...
              <Descriptions.Item label="发放状态">
                {DELIVERY_STATUS_MAP[selectedClaim.delivery_status]?.label ||
                  selectedClaim.delivery_status}
              </Descriptions.Item>
              <Descriptions.Item label="领取时间">
                {formatClaimDate(selectedClaim.claimed_at)}
              </Descriptions.Item>
            </Descriptions>
            {selectedClaim.latest_delivery_id ? (
                ...
                {deliveryDetail ? (
                  <Space orientation="vertical" size="small" className="w-full">
                    <Descriptions size="small" column={1}>
                      <Descriptions.Item label="发放状态">
                        {DELIVERY_STATUS_MAP[deliveryDetail.status]?.label ||
                          deliveryDetail.status}
                      </Descriptions.Item>
                      <Descriptions.Item label="重试次数">
                        {deliveryDetail.retry_count} /{" "}
                        {deliveryDetail.max_retries}
                      </Descriptions.Item>
                      <Descriptions.Item label="下次重试">
                        {formatDate(deliveryDetail.next_retry_at)}
                      </Descriptions.Item>
                      <Descriptions.Item label="更新时间">
                        {formatDate(deliveryDetail.updated_at)}
                      </Descriptions.Item>
                    </Descriptions>
                    <pre className="max-h-56 overflow-auto rounded bg-bg-muted p-3 text-xs">
                      {JSON.stringify(
                        deliveryDetail.external_data || {},
                        null,
                        2
                      )}
                    </pre>
        ...
        >
          {contextCampaignId || editItem?.campaign_id ? (
            <Alert
              ...
              title={
                editItem
                  ? `使用活动：${editItem.campaign_id ? campaignMap[editItem.campaign_id] || "活动" : "未使用"}`
                  : `创建后用于：${selectedCampaign?.name || "当前活动"}`
              }
          ...
          <Form.Item
            ...
            extra="建议使用消费者能理解的名称，例如“20 元复购券”。"
          >
            <Input
              data-testid="benefit-name-input"
              placeholder="例如 20 元复购券"
              maxLength={200}
            />
          </Form.Item>
            ...
            >
              {Object.entries(BENEFIT_TYPE_MAP).map(([value, info]) => (
                <Radio
                  key={value}
                  value={value}
                  className="rounded border border-border-subtle p-3 [&>span:last-child]:w-full"
                >
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <Form.Item
              ...
              extra={
                editItem
                  ? `已领取 ${editItem.stock_used}，总库存不能低于已领取数量。`
                  : "可发放的总数量。"
              }
            >
              <InputNumber
                min={Math.max(editItem?.stock_used || 1, 1)}
                className="w-full"
                data-testid="benefit-stock-input"
                addonAfter="份"
              />
            </Form.Item>
            <Form.Item
              ...
              label="每位消费者限领"
              rules={[{ required: true, message: "请输入每位消费者限领数量" }]}
            >
              <InputNumber
                min={1}
                className="w-full"
                data-testid="benefit-limit-input"
                addonAfter="次"
              />
            </Form.Item>
          </div>
          ...
          {validityType === "after_claim_days" && (
            <Form.Item
              name={["config_json", "validity_days"]}
              label="领取后有效天数"
          ...
          {validityType === "fixed_range" && (
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <Form.Item
                name={["config_json", "validity_start_at"]}
                label="有效期开始"
              ...
              <Form.Item
                name={["config_json", "validity_end_at"]}
                label="有效期结束"
          ...
          {benefitType === "cash_red_packet" && (
            <Alert
              title="微信现金红包"
              description="金额以元为单位输入，保存时会转换为分存储。请确保已配置微信支付转账连接器。"
              type="info"
              showIcon
              className="mb-4"
            />
