...
export function AIDrawer({ open, onClose, form }: AIDrawerProps) {
  ...
  const [pageCopy, setPageCopy] = useState<PageCopyResult["result"] | null>(
    null
  );
  ...
  const handleGeneratePageCopy = async () => {
    ...
    try {
      const result = await generatePageCopy(
        ...
        extractedFields.category || "其他",
        extractedFields.origin ? [extractedFields.origin] : []
      );
  ...
  const applyFieldsToForm = () => {
    ...
    if (extractedFields.product_name)
      updates.name = extractedFields.product_name;
    ...
    if (extractedFields.origin) {
      ...
      if (extractedFields.weight)
        updates.description += `；重量：${extractedFields.weight}`;
      if (extractedFields.shelf_life)
        updates.description += `；保质期：${extractedFields.shelf_life}`;
          ...
          <>
            ...
            <div>
              ...
              <div className="rounded border p-4 space-y-2">
                ...
                {extractedFields.shelf_life && (
                  <div>
                    <Text strong>保质期：</Text>
                    {extractedFields.shelf_life}
                  </div>
                )}
              </div>
            ...
            {pageCopy && (
              <div className="space-y-4 mt-4">
                <div>
                  ...
                  <Paragraph className="mt-1 rounded bg-bg-muted p-3">
                    {pageCopy.copywriting.brand_story}
                  </Paragraph>
                </div>
                <div>
                  ...
                  <List
                    className="mt-1"
                    size="small"
                    dataSource={pageCopy.copywriting.selling_points}
                    renderItem={(item) => (
                      <List.Item>
                ...
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap gap-1">
                    {pageCopy.page_suggestion.modules.map((mod) => (
