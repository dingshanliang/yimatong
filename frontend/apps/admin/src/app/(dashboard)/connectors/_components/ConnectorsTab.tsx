...
export function ConnectorsTab({ connectors, loading, connectorTypes, onRefresh }: ConnectorsTabProps) {
  ...
  const handleEdit = (record: Connector) => {
    setEditing(record);
    form.setFieldsValue({ name: record.name, connector_type: record.connector_type, enabled: record.enabled });
    setModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editing) {
        await api.patch(`/connectors/connectors/${editing.id}`, { name: values.name, enabled: values.enabled });
        message.success("更新成功");
      } else {
        await api.post("/connectors/connectors", { name: values.name, connector_type: values.connector_type, config: {} });
  ...
  const handleTestConnection = async (record: Connector) => {
    try {
      ...
      data.success ? message.success("连接测试成功") : message.warning(`连接测试: ${data.message}`);
    } catch (err) { message.error(extractErrorMessage(err, "连接测试失败")); }
  };
  ...
  const handleSyncStock = async (record: Connector) => {
    try {
      ...
      onRefresh();
    } catch (err) { message.error(extractErrorMessage(err, "库存同步失败")); }
  };
  ...
  return (
    <>
      <div style={{ marginBottom: 16 }}>
        <Button type="primary" icon={<PlusOutlined />} onClick={handleCreate}>新建连接器</Button>
        <Button icon={<ReloadOutlined />} onClick={onRefresh} style={{ marginLeft: 8 }}>刷新</Button>
      </div>
      <Table dataSource={connectors} columns={columns} rowKey="id" loading={loading} pagination={false} />
      <Modal title={editing ? "编辑连接器" : "新建连接器"} open={modalOpen} onOk={handleSubmit} onCancel={() => setModalOpen(false)} okText="保存">
        <Form form={form} layout="vertical">
          <Form.Item name="name" label="连接器名称" rules={[{ required: true, message: "请输入名称" }]}>
            <Input placeholder="如：微信支付商家券" />
          </Form.Item>
          <Form.Item name="connector_type" label="连接器类型" rules={[{ required: true, message: "请选择类型" }]}>
