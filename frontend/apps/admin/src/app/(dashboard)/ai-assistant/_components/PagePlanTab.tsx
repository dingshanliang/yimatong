...
export function PagePlanTab() {
  ...
  const [pageCopyResult, setPageCopyResult] = useState<PageCopyResult | null>(
    null
  );
  const [suggestResult, setSuggestResult] = useState<PageSuggestResult | null>(
    null
  );
  ...
  const handlePageSuggest = async (values: {
    product_name: string;
    category: string;
  }) => {
    setLoading(true);
    ...
    try {
      const data = await suggestPageStructure(
        values.product_name,
        values.category
      );
    ...
    <div>
      ...
      >
        ...
        <Form.Item>
          <Button
            ...
            loading={loading}
            icon={
              activeSubTab === "copy" ? <FileTextOutlined /> : <BulbOutlined />
            }
          >
      ...
      {loading && (
        <div className="py-8 text-center">
          <Spin size="large" />
          <div className="mt-2 text-text-muted">AI 正在分析中...</div>
        </div>
      )}
      ...
      {suggestResult && !loading && activeSubTab === "suggest" && (
        ...
        >
          {suggestResult.suggestion.modules &&
          suggestResult.suggestion.modules.length > 0 ? (
            <div>
