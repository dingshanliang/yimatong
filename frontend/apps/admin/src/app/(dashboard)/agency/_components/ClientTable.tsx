...
interface ClientTableProps {
  ...
  hasError: boolean;
  onFilterChange: (filter: { q?: string; readiness?: string; task_status?: string }) => void;
...
export function ClientTable({
  ...
  const columns: ColumnsType<AgencyClientRow> = [
    ...
    { title: "上线准备度", key: "readiness", render: (_: unknown, record) => (
    ...
    { title: "缺失配置", key: "missing", render: (_: unknown, record) => (
      record.readiness.ready
        ? <Tag color="green">已具备上线条件</Tag>
        : <span>缺：{record.readiness.missing_labels.join("、")}</span>
    )},
    { title: "任务", key: "tasks", render: (_: unknown, record) => (
      <Space size={4} wrap>
        {record.task_summary.overdue > 0 && <Tag color="red">逾期 {record.task_summary.overdue}</Tag>}
        {record.task_summary.pending > 0 && <Tag color="blue">待办 {record.task_summary.pending}</Tag>}
        {record.task_summary.in_progress > 0 && <Tag color="processing">进行中 {record.task_summary.in_progress}</Tag>}
        {record.task_summary.high_priority > 0 && <Tag color="volcano">高优先级 {record.task_summary.high_priority}</Tag>}
        {record.task_summary.pending + record.task_summary.in_progress + record.task_summary.overdue === 0 && <span>—</span>}
      </Space>
    )},
      ...
      <div className="mb-4">
        <Space wrap>
          <Input.Search placeholder="搜索客户名称" allowClear style={{ width: 300 }}
            ...
            onChange={(e) => { if (!e.target.value) onFilterChange({ ...filter, q: undefined }); }}
          />
          <Select placeholder="上线状态" value={filter.readiness || "all"} style={{ width: 180 }}
            options={[
          ...
          />
          <Select placeholder="任务状态" value={filter.task_status || "all"} style={{ width: 160 }}
            options={[
