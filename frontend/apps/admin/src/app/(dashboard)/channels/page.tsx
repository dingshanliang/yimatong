...
export const PROVINCE_CITY_OPTIONS = [
  ...
  { province: "海南", cities: ["海口", "三亚", "三沙", "儋州", "五指山", "琼海", "文昌", "万宁", "东方", "定安", "屯昌", "澄迈", "临高", "白沙", "昌江", "乐东", "陵水", "保亭", "琼中"] },
  ...
  { province: "新疆", cities: ["乌鲁木齐", "克拉玛依", "吐鲁番", "哈密", "昌吉", "博尔塔拉", "巴音郭楞", "阿克苏", "克孜勒苏", "喀什", "和田", "伊犁", "塔城", "阿勒泰", "石河子", "阿拉尔", "图木舒克", "五家渠", "北屯", "铁门关", "双河", "可克达拉", "昆玉", "胡杨河", "新星", "白杨"] },
...
function severityTag(severity?: DiversionClue["severity"]) {
  if (severity === "high") return <Tag color="red">高风险</Tag>;
  if (severity === "medium") return <Tag color="orange">中风险</Tag>;
  return <Tag>低风险</Tag>;
}
...
function batchCapacity(batch: Batch | undefined, allocations: Allocation[]) {
  ...
  const related = allocations.filter((item) => item.batch_id === batch.id || item.batch_code === batch.batch_code);
  const latestRemaining = related.find((item) => typeof item.remaining_quantity === "number")?.remaining_quantity;
  const total = related.find((item) => item.batch_quantity)?.batch_quantity || batch.quantity || 0;
  const allocated = related.reduce((sum, item) => sum + (item.quantity || 0), 0);
  const remaining = typeof latestRemaining === "number" ? latestRemaining : Math.max(total - allocated, 0);
  return { total, allocated: Math.max(total - remaining, allocated), remaining };
}
...
function buildRegionInitialValues(record?: Distributor | Region | Store, defaults?: Record<string, unknown>) {
  const values = { ...(record || {}), ...(defaults || {}) } as Record<string, unknown>;
  if (record && "coverage_type" in record) {
    ...
    if (region.coverage_type === "multi_province") {
      values.coverage_provinces = (region.coverage_areas || []).map((area) => area.province).filter(Boolean);
    }
...
function buildRegionPayload(values: Record<string, unknown>) {
  const coverageType = (values.coverage_type || "city") as Region["coverage_type"];
  ...
  if (coverageType === "province") {
    ...
    const provinces = Array.isArray(values.coverage_provinces) ? (values.coverage_provinces as string[]) : [];
    payload.province = provinces[0];
    payload.city = null;
    payload.coverage_areas = provinces.map((province) => ({ province, city: null }));
  } else {
...
export default function ChannelsPage() {
  ...
  const [createdDistributor, setCreatedDistributor] = useState<Distributor | null>(null);
  ...
  const regionCoverageType = Form.useWatch("coverage_type", entityForm) as Region["coverage_type"] | undefined;
  const regionProvince = Form.useWatch("province", entityForm);
  const regionCity = Form.useWatch("city", entityForm);
  const regionCoverageProvinces = Form.useWatch("coverage_provinces", entityForm) as string[] | undefined;
  ...
  const allocationDistributorId = Form.useWatch("distributor_id", allocationForm);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (clueResolvedFilter !== "all") {
        clueParams.set("resolved", clueResolvedFilter === "resolved" ? "true" : "false");
      }
  ...
  useEffect(() => {
    ...
    entityForm.setFieldsValue({ ...initialValues, status: initialValues.status || "active" });
  }, [createdDistributor, createdRegion, entityForm, entityModal]);

  useEffect(() => {
    if (!entityModal || entityModal.type !== "region" || entityModal.record || createdRegion) return;
    const coverageType = regionCoverageType || "city";
    const provinces = regionCoverageProvinces || [];
    const multiName =
      provinces.length >= 2 ? `${provinces.slice(0, 2).join("、")}${provinces.length > 2 ? "等" : ""}大区` : "";
    ...
    if (suggestedName && (!currentName || currentName === autoRegionNameRef.current)) {
      entityForm.setFieldValue("name", suggestedName);
      autoRegionNameRef.current = suggestedName;
    }
  }, [createdRegion, entityForm, entityModal, regionCity, regionCoverageProvinces, regionCoverageType, regionProvince]);
  ...
  useEffect(() => {
    ...
    if (targetType === "region" && !allocationForm.getFieldValue("region_id") && regions.items[0]) {
  ...
  const regionOptions = useMemo(
    () => regions.items.map((item) => ({ label: `${item.name} / ${regionCoverageLabel(item)}`, value: item.id })),
    [regions.items],
  );
  ...
  const allocationTargetLabel = useMemo(() => {
    ...
    if (allocationTargetType === "distributor" && selectedAllocationDistributor) {
    ...
    return "";
  }, [allocationTargetType, selectedAllocationDistributor, selectedAllocationRegion, selectedAllocationStore]);
  const allocationDistributorName =
    selectedAllocationDistributor?.name || selectedAllocationRegion?.distributor_name || selectedAllocationStore?.distributor_name || "";
  const allocationSummary =
    selectedAllocationBatch && allocationTargetLabel && allocationQuantity
      ? `将 ${selectedAllocationBatch.batch_code} 的 ${allocationQuantity} 个已赋码货品登记到 ${allocationTargetLabel}${
          allocationDistributorName ? `，归属经销商 ${allocationDistributorName}` : ""
        }。`
      : "";
  ...
  const saveEntity = async (values: Record<string, unknown>) => {
    ...
    const payload = entityModal.type === "region" ? buildRegionPayload(mergedValues) : mergedValues;
    const paths = {
    ...
    try {
      if (entityModal.record) {
        await api.patch(`${paths[entityModal.type]}/${entityModal.record.id}`, payload);
        messageRef.current.success("资料已更新");
        closeEntityModal();
      } else {
        const { data } = await api.post(paths[entityModal.type], payload);
        messageRef.current.success(entityModal.type === "distributor" ? "经销商已创建" : "资料已创建");
  ...
  const continueWithScope = () => {
    ...
    closeEntityModal();
    setActiveTab("scopes");
    scopeForm.setFieldsValue({ scope_type: "distributor", distributor_id: distributor.id });
    setScopeOpen(true);
  };
  ...
  const continueWithAllocation = () => {
    ...
    setActiveTab("assign");
    if (createdRegion) {
      allocationForm.setFieldsValue({ target_type: "region", region_id: createdRegion.id, distributor_id: createdRegion.distributor_id });
    }
  ...
  const continueWithRegionScope = () => {
    ...
    closeEntityModal();
    setActiveTab("scopes");
    scopeForm.setFieldsValue({ scope_type: "region", region_id: region.id, distributor_id: region.distributor_id });
    setScopeOpen(true);
  };
  ...
  const resolveClue = async (values: { resolution_action?: string; resolution_note?: string }) => {
    if (!currentClue) return;
    try {
  ...
  const allocationColumns: ColumnsType<Allocation> = [
    ...
    { title: "流向范围", render: (_, record) => record.store_name || record.region_name || record.distributor_name || "未设置范围" },
    { title: "经销商", dataIndex: "distributor_name", render: (value) => value || "未绑定经销商" },
    { title: "登记数量", dataIndex: "quantity", render: (value) => `${value || 0} 个` },
    { title: "批次余量", dataIndex: "remaining_quantity", render: (value) => `剩余 ${value || 0}` },
  ];
  ...
  const clueColumns: ColumnsType<DiversionClue> = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space orientation="vertical" size={0}>
          <Text>{record.store_name || record.region_name || record.distributor_name || "未识别渠道"}</Text>
          <Text type="secondary">{record.distributor_name || "未绑定经销商"}</Text>
        </Space>
      ),
    },
  ...
  const scopeColumns: ColumnsType<AccountScope> = [
    ...
    },
    {
      ...
      render: (value) => (value === "distributor" ? "经销商入口" : value === "region" ? "区域入口" : "门店入口"),
    },
    {
      title: "绑定范围",
      render: (_, record) => {
        ...
        if (record.scope_type === "region") {
          return regions.items.find((item) => item.id === record.region_id)?.name || record.region_id;
        }
    ...
    <div>
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          ...
          <Text type="secondary">维护渠道组织，登记已赋码货品流向，并跟进跨区扫码线索。</Text>
        </div>
      ...
      <Row gutter={[16, 16]} className="mb-5">
        ...
        <Col xs={12} md={8} lg={4}>
          <Card size="small">
            <Statistic title="已登记码量" value={overview?.allocated_quantity || 0} />
          </Card>
        </Col>
        <Col xs={12} md={8} lg={4}>
          <Card size="small">
          ...
          },
          {
            ...
            children: (
              <>
                ...
                <Table columns={clueColumns} dataSource={clues.items} rowKey="id" loading={loading} />
              </>
            ),
          },
        ]}
      ...
      <Modal
        title={entityModal ? `${entityModal.record ? "编辑" : "新建"}${entityTitle}` : undefined}
        open={!!entityModal}
        ...
        {createdDistributor ? (
          <Space orientation="vertical" size={16} className="w-full">
            ...
            <Descriptions column={1} size="small" bordered>
              <Descriptions.Item label="系统编码">{createdDistributor.code}</Descriptions.Item>
              <Descriptions.Item label="联系人">{createdDistributor.contact_name || "未填写"}</Descriptions.Item>
              <Descriptions.Item label="联系电话">{createdDistributor.contact_phone_masked || "未填写"}</Descriptions.Item>
              <Descriptions.Item label="状态">{createdDistributor.status === "inactive" ? "停用" : "启用"}</Descriptions.Item>
            </Descriptions>
            <Space wrap>
          ...
          <Space orientation="vertical" size={16} className="w-full">
            <Alert type="success" showIcon title={`已创建区域：${createdRegion.name}`} description={`系统编码：${createdRegion.code}`} />
            <Descriptions column={1} size="small" bordered>
              <Descriptions.Item label="系统编码">{createdRegion.code}</Descriptions.Item>
              <Descriptions.Item label="省市">
                {[createdRegion.province, createdRegion.city].filter(Boolean).join(" / ") || "未设置"}
              </Descriptions.Item>
              <Descriptions.Item label="所属经销商">{createdRegion.distributor_name || "已绑定经销商"}</Descriptions.Item>
              <Descriptions.Item label="状态">{createdRegion.status === "inactive" ? "停用" : "启用"}</Descriptions.Item>
            </Descriptions>
            <Space wrap>
          ...
          <Form form={entityForm} layout="vertical" onFinish={saveEntity} initialValues={{ status: "active" }}>
            <Form.Item name="name" label="名称" rules={[{ required: true, message: "请输入名称" }]}>
              <Input placeholder={entityModal?.type === "distributor" ? "如华东经销商" : "请输入名称"} />
            </Form.Item>
            {entityModal?.record && (
            ...
            {!entityModal?.record && (
              <Alert className="mb-4" type="info" showIcon title="编码将在创建后自动生成" />
            )}
            {entityModal?.type === "distributor" && (
              <>
            ...
            {entityModal?.type === "region" && (
              <>
                <Form.Item name="coverage_type" label="覆盖类型" rules={[{ required: true, message: "请选择覆盖类型" }]}>
                  <Select
                    options={[
                    ...
                    onChange={() => {
                      entityForm.setFieldsValue({ province: undefined, city: undefined, coverage_provinces: undefined });
                    }}
                  />
                </Form.Item>
                {(regionCoverageType || "city") !== "multi_province" && (
                  <Form.Item name="province" label="省份" rules={[{ required: true, message: "请选择省份" }]}>
                    <Select
                      ...
                      placeholder="选择省份"
                      options={PROVINCE_CITY_OPTIONS.map((item) => ({ label: item.province, value: item.province }))}
                      onChange={() => entityForm.setFieldValue("city", undefined)}
                    />
                  </Form.Item>
                )}
                {(regionCoverageType || "city") === "city" && (
                  <Form.Item name="city" label="城市" rules={[{ required: true, message: "请选择城市" }]}>
                    <Select
                      ...
                      optionFilterProp="label"
                      placeholder="选择城市"
                      options={(PROVINCE_CITY_OPTIONS.find((item) => item.province === regionProvince)?.cities || []).map((city) => ({
                        label: city,
                        value: city,
                      }))}
                ...
                {regionCoverageType === "multi_province" && (
                  <Form.Item
                    ...
                    label="覆盖省份"
                    rules={[{ required: true, type: "array", min: 2, message: "请至少选择两个省份" }]}
                  >
                    <Select
                      mode="multiple"
                      showSearch
              ...
              <>
                <Form.Item name="region_id" label="所属区域">
                  <Select
                    ...
                    onChange={(regionId) => {
                      const region = regions.items.find((item) => item.id === regionId);
                      if (region?.distributor_id) {
                        entityForm.setFieldValue("distributor_id", region.distributor_id);
                      }
                    }}
                  />
                </Form.Item>
      ...
      <Modal
        ...
        destroyOnHidden
      >
        <Form form={allocationForm} layout="vertical" onFinish={createAllocation} initialValues={{ target_type: "region" }}>
          <Alert
            className="mb-4"
            type="info"
            showIcon
          ...
          <Form.Item name="batch_id" label="码批次" rules={[{ required: true, message: "请选择码批次" }]}>
            <Select
              options={batches.map((batch) => ({ label: batch.batch_code, value: batch.id }))}
          ...
          {selectedAllocationBatch && (
            <Card size="small" className="mb-4">
              <Descriptions size="small" column={1}>
                <Descriptions.Item label="批次号">{selectedAllocationBatch.batch_code}</Descriptions.Item>
                <Descriptions.Item label="产品/SKU">
                  {productSkuLabel(selectedAllocationBatch.product_name, selectedAllocationBatch.sku_name)}
                </Descriptions.Item>
              </Descriptions>
              <Space className="mt-2" wrap>
                <Tag>总量 {selectedBatchCapacity.total}</Tag>
                <Tag color="blue">已登记 {selectedBatchCapacity.allocated}</Tag>
                <Tag color="green">剩余 {selectedBatchCapacity.remaining}</Tag>
              </Space>
            </Card>
          )}
          <Form.Item name="target_type" label="流向层级" rules={[{ required: true, message: "请选择流向层级" }]}>
            <Select
              options={[
              ...
              onChange={(value) => {
                allocationForm.setFieldsValue({
                  distributor_id: value === "distributor" ? allocationForm.getFieldValue("distributor_id") : undefined,
                  region_id: value === "region" ? regions.items[0]?.id : undefined,
                  store_id: undefined,
                });
              }}
            />
          ...
          {allocationTargetType === "distributor" && (
            <Form.Item name="distributor_id" label="选择经销商" rules={[{ required: true, message: "请选择经销商" }]}>
              <Select options={distributorOptions} />
            </Form.Item>
          )}
          {allocationTargetType === "region" && (
            <>
              <Form.Item name="region_id" label="选择区域" rules={[{ required: true, message: "请选择区域" }]}>
            ...
            <>
              <Form.Item name="store_id" label="选择门店" rules={[{ required: true, message: "请选择门店" }]}>
                <Select
                  ...
                  onChange={(storeId) => {
                    const store = stores.items.find((item) => item.id === storeId);
                    allocationForm.setFieldsValue({
              ...
              <Alert className="mb-4" type="info" showIcon title="门店入口可查看本次收货批次和扫码趋势，不开放打印或下载码包。" />
            </>
          )}
          <Form.Item label="登记数量" required>
            <Space.Compact className="w-full">
              <Form.Item name="quantity" noStyle rules={[{ required: true, message: "请输入登记数量" }]}>
                <InputNumber
                  aria-label="登记数量"
                  min={1}
              ...
              <Button
                onClick={() => allocationForm.setFieldValue("quantity", selectedBatchCapacity.remaining)}
          ...
          <Text type="secondary">可登记 1-{selectedBatchCapacity.remaining || 0} 个</Text>
          {allocationSummary && <Alert className="mt-4" type="success" showIcon title={allocationSummary} />}
        </Form>
      </Modal>

      <Modal title="绑定入口账号" open={scopeOpen} onCancel={() => setScopeOpen(false)} onOk={() => scopeForm.submit()} forceRender destroyOnHidden>
          ...
          <Form.Item noStyle shouldUpdate>
            {({ getFieldValue }) =>
              getFieldValue("scope_type") === "store" ? (
                ...
                <>
                  <Form.Item name="region_id" label="区域" rules={[{ required: true, message: "请选择区域" }]}>
                    <Select options={regionOptions} />
                  </Form.Item>
                  <Form.Item name="distributor_id" label="所属经销商">
          ...
          <Space orientation="vertical" className="w-full" size={16}>
            ...
            <Card title="关联货品" size="small">
              <Descriptions column={1} size="small" bordered>
                ...
                <Descriptions.Item label="产品/SKU">
                  {productSkuLabel(currentClue.product_name, currentClue.sku_name)}
                </Descriptions.Item>
                <Descriptions.Item label="码批次">{currentClue.batch_code || "未绑定批次"}</Descriptions.Item>
                <Descriptions.Item label="扫码时间">{formatDateTime(currentClue.detected_at)}</Descriptions.Item>
              </Descriptions>
            </Card>
            <Card title="渠道归属" size="small">
              <Descriptions column={1} size="small" bordered>
                <Descriptions.Item label="经销商">{currentClue.distributor_name || "未绑定"}</Descriptions.Item>
                <Descriptions.Item label="区域">{currentClue.region_name || currentClue.expected_region || "未设置"}</Descriptions.Item>
                <Descriptions.Item label="门店">{currentClue.store_name || "未识别门店"}</Descriptions.Item>
                <Descriptions.Item label="状态">{currentClue.resolved ? "已处理" : "待处理"}</Descriptions.Item>
                {currentClue.resolved && (
                  <>
                    <Descriptions.Item label="处理结果">{resolutionActionLabel(currentClue.resolution_action)}</Descriptions.Item>
                    <Descriptions.Item label="处理时间">{formatDateTime(currentClue.resolved_at)}</Descriptions.Item>
                  </>
                )}
              </Descriptions>
            </Card>
            <Alert type="info" showIcon title="建议动作" description={currentClue.handling_recommendation || "联系渠道核实货物流向，记录处理结果。"} />
            <Form form={resolveForm} layout="vertical" onFinish={resolveClue}>
              <Form.Item name="resolution_action" label="处理结果" rules={[{ required: true, message: "请选择处理结果" }]}>
                <Select
              ...
              <Form.Item name="resolution_note" label="处理记录">
                <Input.TextArea rows={4} placeholder="填写处理记录" disabled={currentClue.resolved} />
              </Form.Item>
