...
function clickSelectOption(text: string) {
  const options = Array.from(document.querySelectorAll(".ant-select-item-option-content")).filter(
...
describe("ChannelsPage", () => {
  beforeEach(() => {
    ...
    mockGet.mockImplementation((url: string) => {
      ...
      if (url === "/code-batches") {
        return Promise.resolve(paginated([{ id: "b1", batch_code: "CB-001", quantity: 1000, product_name: "有机大米", sku_name: "5kg" }]));
      }
  ...
  it("renders overview metrics and business-readable master data", async () => {
    ...
    await waitFor(() => expect(screen.getAllByText("已登记码量").length).toBeGreaterThan(0));
  ...
  it("creates distributor without manual code and shows next-step actions", async () => {
    ...
    expect(within(dialog).getByText("编码将在创建后自动生成")).toBeInTheDocument();
    expect(within(dialog).queryByLabelText("编码")).not.toBeInTheDocument();

    fireEvent.change(within(dialog).getByLabelText("名称"), { target: { value: "华南经销商" } });
    fireEvent.change(within(dialog).getByLabelText("联系人"), { target: { value: "张三" } });
    fireEvent.change(within(dialog).getByLabelText("联系电话"), { target: { value: "13800138000" } });
    ...
    expect(within(successDialog).getByRole("button", { name: "创建区域" })).toBeInTheDocument();
    expect(within(successDialog).getByRole("button", { name: "绑定入口账号" })).toBeInTheDocument();
    expect(within(successDialog).getByRole("button", { name: "登记流向" })).toBeInTheDocument();
  }, 10000);
  ...
  it("prefills distributor context and suggests region name from structured city", async () => {
    ...
    const regionDialog = await screen.findByRole("dialog", { name: "新建区域" });
    expect(within(regionDialog).getByText("编码将在创建后自动生成")).toBeInTheDocument();
    ...
    await waitFor(() =>
      expect(
        Array.from(document.querySelectorAll(".ant-select-item-option-content")).some((item) => item.textContent === "上海"),
      ).toBe(true),
    );
    clickSelectOption("上海");

    await waitFor(() => expect(within(regionDialog).getByLabelText("名称")).toHaveValue("上海区域"));
    fireEvent.change(within(regionDialog).getByLabelText("名称"), { target: { value: "上海核心区域" } });
    expect(within(regionDialog).getByLabelText("所属经销商")).toBeInTheDocument();
  }, 10000);
  ...
  it("creates province-level regions without forcing a city", async () => {
    ...
    await waitFor(() => expect(within(regionDialog).queryByLabelText("城市")).not.toBeInTheDocument());

    fireEvent.mouseDown(within(regionDialog).getByLabelText("省份"));
    await screen.findByTitle("北京");
    clickSelectOption("北京");

    await waitFor(() => expect(within(regionDialog).getByLabelText("名称")).toHaveValue("北京省区"));
    expect(within(regionDialog).queryByLabelText("城市")).not.toBeInTheDocument();

    fireEvent.click(within(regionDialog).getByRole("button", { name: "创建区域" }));
  ...
  it("supports multi-province regions for large-area coverage", async () => {
    ...
    await waitFor(() => expect(within(regionDialog).getByLabelText("覆盖省份")).toBeInTheDocument());

    expect(within(regionDialog).queryByLabelText("省份")).not.toBeInTheDocument();
    expect(within(regionDialog).queryByLabelText("城市")).not.toBeInTheDocument();
    ...
    await waitFor(() => expect(within(regionDialog).getByLabelText("名称")).toHaveValue("北京、天津等大区"));
    fireEvent.click(within(regionDialog).getByRole("button", { name: "创建区域" }));
  ...
  it("contains complete province city options for region creation", () => {
    const byProvince = new Map(PROVINCE_CITY_OPTIONS.map((item) => [item.province, item.cities]));
  ...
  it("shows allocation form with readable batch capacity and submission summary", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("dialog", { name: "新建流向登记" })).toBeInTheDocument());
    const allocationDialog = screen.getByRole("dialog", { name: "新建流向登记" });

    expect(within(allocationDialog).getByText("记录这批已赋码货品发往哪个渠道，不开放打印或下载码包。")).toBeInTheDocument();
    expect(within(allocationDialog).getByText("批次号")).toBeInTheDocument();
    expect(within(allocationDialog).getAllByText("CB-001").length).toBeGreaterThanOrEqual(2);
    expect(within(allocationDialog).getByText("产品/SKU")).toBeInTheDocument();
    expect(within(allocationDialog).getByText("有机大米 / 5kg")).toBeInTheDocument();
    expect(within(allocationDialog).getByText("总量 1000")).toBeInTheDocument();
    expect(within(allocationDialog).getByText("已登记 300")).toBeInTheDocument();
    expect(within(allocationDialog).getByText("剩余 700")).toBeInTheDocument();
    expect(within(allocationDialog).getByLabelText("流向层级")).toBeInTheDocument();
    expect(within(allocationDialog).getByLabelText("选择区域")).toBeInTheDocument();
    expect(within(allocationDialog).getByText("系统已根据区域带出经销商：华东经销商")).toBeInTheDocument();
    expect(within(allocationDialog).getByText("可登记 1-700 个")).toBeInTheDocument();

    fireEvent.click(within(allocationDialog).getByRole("button", { name: "全部登记" }));
    await waitFor(() => expect(within(allocationDialog).getByRole("spinbutton", { name: "登记数量" })).toHaveValue("700"));
    expect(within(allocationDialog).getByText("将 CB-001 的 700 个已赋码货品登记到 上海区域 / 上海，归属经销商 华东经销商。")).toBeInTheDocument();
  });
  ...
  it("uses clear allocation target labels and keeps store target discoverable", async () => {
    ...
    const allocationButton = await screen.findByText("新建流向");
    fireEvent.click(allocationButton.closest("button") as HTMLButtonElement);
    const allocationDialog = await screen.findByRole("dialog", { name: "新建流向登记" });

    fireEvent.mouseDown(within(allocationDialog).getByLabelText("流向层级"));
    await waitFor(() => expect(screen.getAllByTitle("区域").length).toBeGreaterThan(1));
    expect(await screen.findByTitle("门店")).toBeInTheDocument();
    clickSelectOption("门店");

    expect(await within(allocationDialog).findByLabelText("选择门店")).toBeInTheDocument();
    expect(within(allocationDialog).getByText("门店入口可查看本次收货批次和扫码趋势，不开放打印或下载码包。")).toBeInTheDocument();
  });
