...
const columns: ColumnsType<Store> = [
  ...
  },
  {
    ...
    render: (v: string) => (
      <Tag
        color={v === "active" ? STATUS_COLORS.success : STATUS_COLORS.neutral}
      >
        {v === "active" ? "活跃" : v}
      </Tag>
    ),
...
export function StoreTab() {
  const { items, total, page, loading, setPage, create, update, mutate } =
    useCrud<Store>("/channels/stores");
  ...
  useEffect(() => {
    api
      .get("/channels/regions?page_size=100")
      .then(({ data }) => {
      ...
      .catch(() => {});
    api
      .get("/channels/distributors?page_size=100")
      .then(({ data }) => {
        const items = data?.items || data || [];
        setDistOptions(
