...
const columns: ColumnsType<Order> = [
  ...
  { title: "渠道", dataIndex: "channel", key: "channel", render: (v: string) => v ? <Tag>{v}</Tag> : "—" },
  { title: "来源", dataIndex: "source_system", key: "source_system", render: (v: string) => v || "—" },
  { title: "匹配状态", dataIndex: "matched", key: "matched", render: (v: boolean) => <Tag color={v ? "green" : "default"}>{v ? "已归因" : "待匹配"}</Tag> },
];

export function OrdersTab() {
  const [filter, setFilter] = useState<Record<string, string>>({});
  const { items, total, page, loading, setPage, mutate, setFilter: setCrudFilter } = useCrud<Order>("/gmv/orders");
  const [importOpen, setImportOpen] = useState(false);
  const [form] = Form.useForm();

  const handleImport = async (values: Record<string, unknown>) => {
    try {
      const orders = values.orders_json ? JSON.parse(values.orders_json as string) : [];
      ...
      <Modal title="导入外部订单" open={importOpen} onCancel={() => setImportOpen(false)} onOk={() => form.submit()} width={600}>
        <Form form={form} layout="vertical" onFinish={handleImport}>
          <Form.Item name="orders_json" label="订单数据 (JSON 数组)" rules={[{ required: true }]}>
            <Input.TextArea rows={8} placeholder='[{"external_id":"ORD001","amount":99.9,"phone":"13800138000","channel":"taobao","source_system":"erp"}]' />
          </Form.Item>
        </Form>
      </Modal>
    </>
