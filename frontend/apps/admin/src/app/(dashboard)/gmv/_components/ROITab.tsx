...
const columns: ColumnsType<ROIItem> = [
  { title: "活动名称", dataIndex: "campaign_name", key: "campaign_name", width: 160 },
  {
    title: "状态", dataIndex: "status", key: "status", width: 80,
    render: (v: string) => {
      const map: Record<string, string> = { active: "green", draft: "default", paused: "orange", ended: "red" };
      return <Tag color={map[v] || "default"}>{v}</Tag>;
    },
  },
  { title: "预算", dataIndex: "budget", key: "budget", width: 100, render: (v: number | null) => v ? `¥${v.toLocaleString()}` : "—" },
  { title: "归因 GMV", dataIndex: "attributed_gmv", key: "attributed_gmv", width: 110, render: (v: number) => <Text strong>¥{v.toLocaleString()}</Text> },
  ...
  },
  {
    ...
    render: (v: number) => {
      ...
      return <Text type={color} strong>{v}x</Text>;
    },
  },
  {
    title: "置信度", dataIndex: "avg_confidence", key: "avg_confidence", width: 80,
    render: (v: number) => <Text type={v >= 0.8 ? "success" : "warning"}>{(v * 100).toFixed(0)}%</Text>,
  },
];
...
export function ROITab() {
  ...
  const avgConversion = data.length ? data.reduce((s, r) => s + r.conversion_rate, 0) / data.length : 0;
  ...
  return (
    <div>
      ...
      <Row gutter={[16, 16]} className="mb-4">
        <Col span={6}>
          <Card size="small"><Statistic title="总归因 GMV" value={totalGmv} prefix="¥" loading={loading} /></Card>
        </Col>
        <Col span={6}>
          <Card size="small"><Statistic title="总归因订单" value={totalOrders} loading={loading} /></Card>
        </Col>
        <Col span={6}>
