...
export default function StorePortalPage() {
  ...
  if (error) {
    return <Alert type="warning" message="未找到门店入口范围" description="请联系品牌方管理员绑定门店范围。" />;
  }
  ...
  return (
    <div>
      <div className="mb-5">
        ...
        <Text type="secondary">{summary?.scope.name || "正在加载门店数据"}</Text>
      </div>
      ...
      <Row gutter={[16, 16]} className="mb-5">
        ...
        <Col xs={24} md={8}>
          <Card size="small">
            <Statistic title="门店状态" value={summary?.status === "active" ? "启用" : "停用"} loading={loading} />
          </Card>
        </Col>
      </Row>
      ...
      <Card title="门店资料" size="small" className="mb-5">
        <Descriptions column={1} size="small">
          <Descriptions.Item label="门店编码">{summary?.scope.code || "-"}</Descriptions.Item>
          <Descriptions.Item label="所属区域">{summary?.region_name || "未绑定"}</Descriptions.Item>
          <Descriptions.Item label="所属经销商">{summary?.distributor_name || "未绑定"}</Descriptions.Item>
          <Descriptions.Item label="地址">{summary?.address || "未填写"}</Descriptions.Item>
        </Descriptions>
      </Card>
