...
export default function TenantsPage() {
  ...
  const handleStatusChange = async (tenant: Tenant, newStatus: string) => {
    const statusLabels: Record<string, string> = { suspended: "暂停", active: "恢复", terminated: "终止" };
    modal.confirm({
      title: `确认${statusLabels[newStatus]}租户`,
      content: `确定要${statusLabels[newStatus]}租户「${tenant.name}」吗？`,
      onOk: async () => {
        try {
          await api.patch(`/platform/tenants/${tenant.id}/status`, { status: newStatus });
  ...
  const columns: ColumnsType<Tenant> = [
    ...
    { title: "Slug", dataIndex: "slug", key: "slug", width: 120, ellipsis: true },
    {
      title: "套餐",
      dataIndex: "plan",
      key: "plan",
    ...
    },
    {
      ...
      render: (_: unknown, record: Tenant) => {
        const items: { key: string; icon: React.ReactNode; label: string; onClick: () => void; danger?: boolean }[] = [
          { key: "view", icon: <EyeOutlined />, label: "查看详情", onClick: () => router.push(`/tenants/${record.id}`) },
        ];
        if (record.status === "active") {
          items.push({ key: "suspend", icon: <PauseCircleOutlined />, label: "暂停", onClick: () => handleStatusChange(record, "suspended") });
        }
        if (record.status === "suspended") {
          items.push({ key: "resume", icon: <PlayCircleOutlined />, label: "恢复", onClick: () => handleStatusChange(record, "active") });
        }
        if (record.status !== "terminated") {
          items.push({ key: "terminate", icon: <DeleteOutlined />, label: "终止", danger: true, onClick: () => handleStatusChange(record, "terminated") });
        }
        return (
    ...
    <div>
      ...
      <Card>
        <Space style={{ marginBottom: 16 }} wrap>
          ...
          <Select
            ...
            options={Object.entries(PLAN_MAP).map(([k, v]) => ({ value: k, label: v.label }))}
          />
        </Space>
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleCreate}>
          ...
          <Form.Item name="slug" label="Slug" rules={[{ required: true, message: "请输入 Slug" }, { pattern: /^[a-z0-9-]+$/, message: "只允许小写字母、数字和连字符" }]}>
            <Input placeholder="例：moumou-food" />
          </Form.Item>
          ...
          <Typography.Text strong style={{ display: "block", marginBottom: 16 }}>管理员账号</Typography.Text>
          <Form.Item name="admin_name" label="管理员姓名" rules={[{ required: true }]}>
          ...
          <Form.Item name="admin_password" label="管理员密码" rules={[{ required: true, min: 8 }]}>
