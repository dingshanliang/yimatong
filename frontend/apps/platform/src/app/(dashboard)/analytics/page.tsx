...
export default function AnalyticsPage() {
  const { data: healthTenants } = useSWR<HealthTenant[]>(
  ...
  const totalScans30d =
    healthTenants?.reduce((sum, t) => sum + t.scans_last_30d, 0) ?? 0;
  const totalTenants = healthTenants?.length ?? 0;
  const avgScore = healthTenants?.length
    ? Math.round(
        healthTenants.reduce((sum, t) => sum + t.health_score, 0) /
          healthTenants.length
      )
    : 0;
                ...
                {topTenants.map((tenant, i) => (
                  ...
                  >
                    ...
                    <span style={{ fontWeight: "bold" }}>
                      {tenant.scans_last_30d.toLocaleString()}
                    </span>
                    <span
                      style={{
                        marginLeft: 8,
            ...
            {overview && (
              ...
              >
                <div style={{ textAlign: "center" }}>
                  <div
                    style={{
                      fontSize: "var(--ymt-font-size-3xl)",
                      fontWeight: "bold",
                      color: "var(--ymt-color-feedback-success)",
                    }}
                  >
                    {overview.healthy}
                  </div>
                ...
                <div style={{ textAlign: "center" }}>
                  <div
                    style={{
                      fontSize: "var(--ymt-font-size-3xl)",
                      fontWeight: "bold",
                      color: "var(--ymt-color-feedback-warning)",
                    }}
                  >
                    {overview.warning}
                  </div>
