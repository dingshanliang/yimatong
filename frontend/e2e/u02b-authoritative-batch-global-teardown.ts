...
interface CleanupDependencies {
  readLease: () => Promise<U02bLease>;
  removeDatabase: (lease: U02bLease) => Promise<boolean>;
  recoverTargetDatabase: (lease: U02bLease) => Promise<void>;
  ...
  try {
    ...
    try {
      ...
      throw new AggregateError(
        ...
        `U02B owned database cleanup failed for ${lease.databaseName}`
      );
    }
  }
}

export async function cleanupU02bResources(
  dependencies: CleanupDependencies
): Promise<{ lease: U02bLease | null; removed: boolean | null }> {
  const errors: unknown[] = [];
  let lease: U02bLease | null = null;
  ...
  if (errors.length > 0) {
    throw new AggregateError(errors, "U02B resource cleanup was incomplete");
  }
...
export default async function u02bAuthoritativeBatchGlobalTeardown() {
  ...
  const validRole =
    ...
    /^u02b_control_[a-f0-9]{12}$/.test(controlRole);

  const result = await cleanupU02bResources({
    readLease: async () => {
      if (!leaseFile) {
        throw new Error("Refusing U02B database cleanup without a lease file");
      }
      return (await readLease(leaseFile)) as U02bLease;
    },
    ...
    recoverTargetDatabase: async (lease) => {
      if (!validRole) {
        throw new Error("Refusing U02B recovery without a valid control role");
      }
      await runPsql(
        composeFile,
        lease.databaseName,
    ...
    removeControlRole: async () => {
      if (!validRole) {
        throw new Error(
          "Refusing U02B role cleanup without a valid control role"
        );
      }
      await runPsql(
        composeFile,
    ...
    onRecovered: (firstError) => {
      console.warn(
        `[u02b] Recovered owned database cleanup after the first attempt failed: ${firstError}`
      );
    },
  });

  if (!result.lease) {
    throw new Error("U02B cleanup completed without a validated lease");
  }
  console.log(
    `[u02b] Teardown ${result.removed ? "removed" : "confirmed absent"} owned database ` +
