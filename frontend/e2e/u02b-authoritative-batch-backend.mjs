...
const ownerToken = process.env.YIMATONG_U02B_OWNER_TOKEN;
const leaseFile = process.env.YIMATONG_U02B_LEASE_FILE;
const controlRole = process.env.YIMATONG_U02B_CONTROL_ROLE;
const controlRolePassword = randomBytes(32).toString("hex");
const apiPort = process.env.YIMATONG_U02B_API_PORT || "18230";
const adminOrigin =
  process.env.YIMATONG_U02B_ADMIN_ORIGIN || "http://127.0.0.1:13230";
const h5Origin =
  process.env.YIMATONG_U02B_H5_ORIGIN || "http://127.0.0.1:13231";

if (!databaseName || !ownerToken || !leaseFile || !controlRole) {
  throw new Error(
    "U02B database name, owner token, lease file, and control role are required"
  );
}
if (!/^u02b_control_[a-f0-9]{12}$/.test(controlRole)) {
  throw new Error("Refusing to use an invalid U02B control role");
}
...
const backendEnvironment = {
  ...
  redis_url: "redis://127.0.0.1:6380/11",
  secret_key: "u02b-browser-secret-key-0123456789-ABCDEFGHIJKLMNOPQRSTUVWXYZ",
  AES_MASTER_KEY_V1: "d4".repeat(32),
  HMAC_PEPPER: "e5".repeat(32),
  IP_HASH_SECRET:
    "u02b-browser-ip-hash-secret-0123456789-ABCDEFGHIJKLMNOPQRSTUVWXYZ",
  base_url: `http://127.0.0.1:${apiPort}`,
  admin_public_url: adminOrigin,
  platform_public_url: "http://127.0.0.1:13232",
  cors_origins: `${adminOrigin},${h5Origin}`,
  cookie_secure: "false",
  cookie_samesite: "lax",
};
...
async function removeDatabaseWithRecovery() {
  try {
    ...
    try {
      ...
      console.warn(
        `[u02b] Recovered database cleanup after first failure: ${firstError}`
      );
      return removed;
    } catch (retryError) {
      throw new AggregateError(
        [firstError, ...(dependencyError ? [dependencyError] : []), retryError],
        `U02B owned database cleanup failed for ${databaseName}`
      );
    }
  }
}
  ...
  await composePsql(
    ...
    `INSERT INTO accounts (
        ...
        AND NOT EXISTS (
          SELECT 1 FROM accounts
           WHERE accounts.tenant_id = tenants.id
             AND accounts.email = 'viewer.u02b@demo.com'
        );
     ...
     SELECT accounts.tenant_id, accounts.id, roles.id
      ...
      WHERE accounts.email = 'viewer.u02b@demo.com'
     ON CONFLICT DO NOTHING;
     INSERT INTO brands (id, tenant_id, name, status, created_at, updated_at)
     SELECT '00000000-0000-7000-8000-0000000002b0'::uuid,
            tenants.id, 'U02B 外租户品牌', 'active', now(), now()
       FROM tenants
     ...
     SELECT '00000000-0000-7000-8000-0000000002b2'::uuid,
            tenants.id, '00000000-0000-7000-8000-0000000002b0'::uuid,
            'U02B 外租户产品', 'active', now(), now()
       FROM tenants
      WHERE tenants.slug = 'demo-agency'
     ON CONFLICT DO NOTHING;
     INSERT INTO skus (
     ...
     SELECT '00000000-0000-7000-8000-0000000002b3'::uuid,
            tenants.id, '00000000-0000-7000-8000-0000000002b2'::uuid,
            'U02B-FOREIGN', 'U02B 外租户 SKU', 'active', now(), now()
       FROM tenants
      WHERE tenants.slug = 'demo-agency'
     ON CONFLICT DO NOTHING;
     ...
     SELECT '00000000-0000-7000-8000-0000000002b1'::uuid,
     ...
     BEGIN
       IF NOT EXISTS (
         ...
         RAISE EXCEPTION 'U02B viewer seed is incomplete';
       END IF;
       IF NOT EXISTS (
         ...
         RAISE EXCEPTION 'U02B agency authorization is incomplete';
       END IF;
     END $$;`
  );
}
...
async function performCleanup() {
  ...
  try {
    if (lease) {
      ...
      console.log(
        `[u02b] ${removed ? "Removed" : "Confirmed absent"} owned database ` +
  ...
  try {
    ...
    console.log(`[u02b] Removed or confirmed absent role ${controlRole}`);
  } catch (error) {
    errors.push(error);
  }
  ...
  if (errors.length > 0) {
    console.error(
      `[u02b] Cleanup failed for ${databaseName}`,
      new AggregateError(errors, "U02B resource cleanup was incomplete")
    );
    process.exitCode = 1;
  } else {
...
try {
  ...
  console.log(`[u02b] Prepared dedicated batch database ${databaseName}`);
  ...
  backendProcess.once("error", (error) => {
    console.error("[u02b] Backend failed to start", error);
    void cleanup(1);
  });
  backendProcess.once("exit", (code, signal) => {
    if (!cleaningUp) {
      console.error(
        `[u02b] Backend exited unexpectedly with ${code ?? signal}`
      );
      void cleanup(code || 1);
    }
  });
} catch (error) {
  console.error("[u02b] Browser backend setup failed", error);
  await cleanup(1);
}
