...
export default async function u01eAgencyAuthorizationGlobalTeardown() {
  const leaseFile = process.env.YIMATONG_U01E_LEASE_FILE;
  const controlRole = process.env.YIMATONG_U01E_CONTROL_ROLE;
  if (
    !leaseFile ||
    !controlRole ||
    !/^u01e_control_[a-f0-9]{12}$/.test(controlRole)
  ) {
    throw new Error(
      "Refusing U01E cleanup without a valid lease and control role"
    );
  }
  ...
  console.log(
    `[u01e] Teardown ${removed ? "removed" : "confirmed absent"} owned database ${lease.databaseName}; cluster=${lease.clusterSystemIdentifier}; marker=${lease.marker}; control_role=${controlRole}`
  );
}
