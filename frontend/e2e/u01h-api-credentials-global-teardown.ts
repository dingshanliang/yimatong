...
export default async function u01hApiCredentialsGlobalTeardown() {
  const leaseFile = process.env.YIMATONG_U01H_LEASE_FILE;
  const controlRole = process.env.YIMATONG_U01H_CONTROL_ROLE;
  if (
    !leaseFile ||
    !controlRole ||
    !/^u01h_control_[a-f0-9]{12}$/.test(controlRole)
  ) {
    throw new Error(
      "Refusing U01H cleanup without a valid lease and control role"
    );
  }
  ...
  console.log(
    `[u01h] Teardown ${removed ? "removed" : "confirmed absent"} owned database ` +
      `${lease.databaseName}; cluster=${lease.clusterSystemIdentifier}; ` +
      `marker=${lease.marker}; control_role=${controlRole}`
  );
}
