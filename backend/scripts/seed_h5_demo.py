...
def main():
    # 1. 创建租户
    r = requests.post(f"{BASE}/api/v1/tenants", json={
        "name": "好味道食品",
    ...
    # 5. 创建 SKU
    r = requests.post(f"{BASE}/api/v1/skus", json={
    ...
    # 6. 创建生产批次
    r = requests.post(f"{BASE}/api/v1/production-batches", json={
        "product_id": product_id,
        "sku_id": sku_id,
    ...
    # 7. 创建页面模板并发布
    r = requests.post(f"{BASE}/api/v1/page-templates", json={
        "name": "五常大米扫码页",
    ...
    config = {
        "modules": [
            ...
            {"id": "trace", "type": "light_traceability", "enabled": True, "config": {"fields": ["origin", "production_date", "batch_code"]}},
            {"id": "cta", "type": "cta_group", "enabled": True, "config": {"buttons": [
                {"label": "加入企业微信", "action": "wecom_link", "url": "https://example.com/wecom"},
                {"label": "去京东购买", "action": "external_shop", "url": "https://jd.com"},
            ]}},
            {"id": "lead", "type": "lead_form", "enabled": True, "config": {"title": "留下联系方式", "fields": ["phone", "region"]}},
        ],
    }

    r = requests.post(f"{BASE}/api/v1/page-templates/{template_id}/versions", json={
        "config_json": config,
    }, headers=headers)
    ...
    # 8. 生成码批次
    r = requests.post(f"{BASE}/api/v1/code-batches", json={
        "product_id": product_id,
        "sku_id": sku_id,
    ...
    print(f"\n💡 下一步:")
    print(f"   1. 用二维码生成器把上面的 URL 生成二维码")
    print(f"   2. 或直接在手机浏览器打开上面的 URL")
    print(f"   3. 或启动 H5 前端: cd frontend && pnpm dev:h5")
