...
from app.services.channel import _region_matches_detected_city, list_diversion_clues
...
class TestRegionMatchesDetectedCity:
    ...
    def test_exact_city_match(self):
        """城市精确匹配"""
        region = Region(
            city="上海", province="上海", coverage_type="city", coverage_areas=[{"province": "上海", "city": "上海"}]
        )
    ...
    def test_coverage_area_match(self):
        ...
        region = Region(
            ...
            coverage_areas=[{"province": "浙江", "city": "杭州"}, {"province": "江苏", "city": "南京"}],
        )
...
@pytest.mark.anyio
async def test_list_diversion_clues_supports_high_severity_filter():
    ...
    rows, total = await list_diversion_clues(
        db,
        uuid.uuid4(),
        severity="high",
    )
