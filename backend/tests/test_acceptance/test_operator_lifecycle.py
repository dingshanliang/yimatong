...
class TestConcurrentSafety:
    ...
    async def test_repeated_freeze_idempotent(self, client, bypass_session, migrated_pg_url):
        ...
        headers = {"Authorization": f"Bearer {token}"}
        # 第一次冻结
        r1 = await client.post(f"/api/v1/risk-alerts/code-items/{item_id}/freeze", headers=headers)
        assert r1.status_code == 200
        assert r1.json()["status"] == "frozen"

        # 第二次冻结（重复）— 应 409（状态机拒绝）或幂等 200，但不能崩
        r2 = await client.post(f"/api/v1/risk-alerts/code-items/{item_id}/freeze", headers=headers)
        assert r2.status_code in (200, 409), f"重复冻结应 200/409，实际 {r2.status_code}"
