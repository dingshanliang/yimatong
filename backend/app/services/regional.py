...
async def create_regional_org(
    db: AsyncSession, tenant_id: uuid.UUID, name: str, org_type: str,
) -> RegionalOrg:
...
async def add_member(
    db: AsyncSession, org_id: uuid.UUID, tenant_id: uuid.UUID, member_name: str,
) -> RegionalOrgMember:
...
async def create_shared_template(
    db: AsyncSession, org_id: uuid.UUID, name: str, config: dict,
) -> RegionalTemplate:
...
async def authorize_product(
    db: AsyncSession, org_id: uuid.UUID, product_id: uuid.UUID, tenant_id: uuid.UUID,
) -> RegionalProductAuth:
...
async def get_regional_dashboard(
    ...
    member_tids_result = await db.execute(
        select(RegionalOrgMember.tenant_id).where(RegionalOrgMember.org_id == org_id)
    )
...
async def create_code_rule(
    db: AsyncSession, org_id: uuid.UUID, rule_name: str, pattern: str, prefix: str,
) -> RegionalCodeRule:
...
async def get_advanced_dashboard(
    ...
    members_result = await db.execute(
        select(RegionalOrgMember).where(RegionalOrgMember.org_id == org_id)
    )
    members = list(members_result.scalars().all())
    member_stats = [
        {"tenant_id": str(m.tenant_id), "name": m.member_name, "status": m.status}
        for m in members
    ]
...
async def set_whitelabel(
    db: AsyncSession, org_id: uuid.UUID, brand_name: str, hide_yimatong: bool, primary_color: str = "#000000",
) -> WhitelabelConfig:
    result = await db.execute(
